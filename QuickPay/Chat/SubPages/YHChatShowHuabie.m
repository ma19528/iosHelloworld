...
@interface YHChatShowHuabie () <UINavigationControllerDelegate, UIImagePickerControllerDelegate> {
...
@implementation YHChatShowHuabie
...
- (void)setupNavigationBar {
    ...
    self.navigationController.navigationBar.backgroundColor = kHuabieColor;
    self.navigationController.navigationBar.barTintColor    = kHuabieColor;
    // 去掉状态栏一像素的横线
    [self.navigationController.navigationBar setShadowImage:[UIImage new]];
    self.view.backgroundColor = kHuabieColor;
...
- (void)setupTypeQrcodeUI {
    ...
    //_imgQcodeIconPayType.image = [UIImage imageNamed:@"icon_huabie"];
    ...
    _imgQrCodeIcon.image = [UIImage imageNamed:@"icon_huabie"];
...
- (void)setupTypeAccountsUI {
    ...
    //_imgIconPayType.image = [UIImage imageNamed:@"icon_huabie"];
...
- (void)setContents {
    ...
    _imgQcodeIconPayType.image = [UIImage imageNamed:@"icon_huabie"]; // 二维码标题的icon。
    _imgIconPayType.image = [UIImage imageNamed:@"icon_huabie"];

    _lbQcodeTitlePayType.text = @"蚂蚁花呗二维码收款";
    _lbQcodePayTipsPayType.text = @"保存二维码到相册，支付宝[扫一扫]付钱";

    _lbPayFullNameT.text = @"银行卡姓名";
    _lbPayFullNameContens.text = [_payInfoModel.firstName stringByAppendingString: _payInfoModel.lastName] ;

    _lbTitlePayType.text = @"银行卡收款";
    _lbPayTipsPayType.text = @"可使用支付宝，微信[转账到银行卡]功能付钱";

    _lbPayAccountsT.text = @"银行卡号";
    ...
    if (_displayType == Show_Qrcode) {
        _lbTipsContents1.text = arrayHuabeiTips[0];
        _lbTipsContents2.text = arrayHuabeiTips[1];
