...
@interface YHChatShowBank () <UINavigationControllerDelegate, UIImagePickerControllerDelegate> {
...
- (void)setupNavigationBar {
    ...
    self.navigationController.navigationBar.backgroundColor = kBankColor;
    self.navigationController.navigationBar.barTintColor    = kBankColor;
    // 去掉状态栏一像素的横线
    [self.navigationController.navigationBar setShadowImage:[UIImage new]];
    self.view.backgroundColor = kBankColor;
...
- (void)setupTypeQrcodeUI {
    ...
    //_imgQcodeIconPayType.image = [UIImage imageNamed:@"icon_bank"];
    ...
    _imgQrCodeIcon.image = [UIImage imageNamed:@"icon_bank"];
...
- (void)setContents {
    ...
    _lbQcodeTitlePayType.text = @"银行卡二维码收款";
    _lbQcodePayTipsPayType.text = @"保存二维码到相册，银联闪付[扫一扫]付钱";
    ...
    if (_displayType == Show_Qrcode) {
        ...
        _lbTipsContents1.text = arrayBankTips[0];
        _lbTipsContents2.text = arrayBankTips[1];
        _lbTipsContents3.text = arrayBankTips[2];
        [_lbTipsContents4 setHidden:YES];
    } else {
