...
* ****/
    - (void)setContents {
        ...
        NSInteger payType = 0;    //支付类型。
        NSInteger dispayType = 0; // 显示类型。0：二维码+账号，1： 二维码显示，2：账号。

        if (dispayType ==0 || dispayType == 3) {
            // 二维码+账号显示。
        } else if (dispayType == 1) {
            // 1： 二维码显示
        } else if(dispayType == 2) {
            // 2：账号。
        }
