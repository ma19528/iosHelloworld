...
@implementation YHChatShowWePay
...
- (void)setupNavigationBar {
    ...
    self.navigationController.navigationBar.backgroundColor = kWechatColor;
    self.navigationController.navigationBar.barTintColor    = kWechatColor;
    // 去掉状态栏一像素的横线
    [self.navigationController.navigationBar setShadowImage:[UIImage new]];
    self.view.backgroundColor = kWechatColor;
...
- (void)setupTypeQrcodeUI {
    ...
    //_imgQcodeIconPayType.image = [UIImage imageNamed:@"icon_wechat"];
    ...
    _imgQrCodeIcon.image = [UIImage imageNamed:@"icon_wechat"];
...
- (void)setupTypeAccountsUI {
    ...
    //_imgIconPayType.image = [UIImage imageNamed:@"icon_wechat"];
...
- (void)setContents {
    ...
    _imgQcodeIconPayType.image = [UIImage imageNamed:@"icon_wechat"]; // 二维码标题的icon。
    _imgIconPayType.image = [UIImage imageNamed:@"icon_wechat"];

    _lbQcodeTitlePayType.text = @"微信二维码收款";
    _lbQcodePayTipsPayType.text = @"保存二维码到相册，微信[扫一扫]付钱";

    _lbTitlePayType.text = @"微信账号转帐";
    _lbPayTipsPayType.text = @"复制微信账号，微信[转账]付钱";

    _lbPayAccountsT.text = @"微信账号";
    _lbPayAccountsContens.text = _payInfoModel.accounts;

    _lbPayFirstNameT.text = @"微信姓";
    ...
    if (_displayType == Show_Qrcode) {
        _lbTipsContents1.text = @"*如有充值问题，请咨询专员*",
                _lbTipsContents2.text = @"为避免扫码失败，使用微信扫码之前，请关闭wifi，用手机流量来扫码支付";
        [_lbTipsContents3 setHidden:YES];
        [_lbTipsContents4 setHidden:YES];
    } else if (_displayType == Show_Account) {
        _lbTipsContents1.text = arrayWechatTips[0];
        _lbTipsContents2.text = arrayWechatTips[1];
        _lbTipsContents3.text = @"微信转到银行卡功能路径：打开微信>右上角+号>收付款>转到银行卡";
        [_lbTipsContents3 setHidden:YES];
        [_lbTipsContents4 setHidden:YES];
    } else {
