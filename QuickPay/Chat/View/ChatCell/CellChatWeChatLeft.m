...
@implementation CellChatWeChatLeft
...
- (void)setupModel:(YHChatModel *)model{
    ...
    if (self.model.msgType == YHMessageType_WECHAT && msgContent) {
        self.imgvIcon.image = [UIImage imageNamed:@"chat_pay_wechat"];
    }
...
- (void)onBtnTapScope:(UIButton *)sender{
    if (_delegate && [_delegate respondsToSelector:@selector(onChatWeChat:inLeftCell:)]) {
        [_delegate onChatWeChat:self.model inLeftCell:self];
    }
}
