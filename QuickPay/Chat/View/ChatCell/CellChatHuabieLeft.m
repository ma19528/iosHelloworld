...
@implementation CellChatHuabieLeft
...
- (void)setupModel:(YHChatModel *)model{
    ...
    if (self.model.msgType == YHMessageType_HUABIE && msgContent) {
        self.imgvIcon.image = [UIImage imageNamed:@"chat_pay_huabie"];
    }
...
- (void)onBtnTapScope:(UIButton *)sender{
    if (_delegate && [_delegate respondsToSelector:@selector(onChatWeChat:inLeftCell:)]) {
        [_delegate onChatHuabie:self.model inLeftCell:self];
    }
}
