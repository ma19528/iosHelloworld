...
@implementation CellChatCreditLeft
...
- (void)setupModel:(YHChatModel *)model{
    ...
    if (self.model.msgType == YHMessageType_CREDIT && msgContent) {
        self.imgvIcon.image = [UIImage imageNamed:@"chat_pay_credit"];
    }
...
- (void)onBtnTapScope:(UIButton *)sender{
    if (_delegate && [_delegate respondsToSelector:@selector(onChatWeChat:inLeftCell:)]) {
        [_delegate onChatCredit:self.model inLeftCell:self];
    }
}
